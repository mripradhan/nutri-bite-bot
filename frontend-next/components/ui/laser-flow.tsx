...
const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRGB(color || '#34d59a')
    ...
    const adjustDprIfNeeded = (now: number) => {
      ...
      if (avgFps < 50) next = clamp(currentDprRef.current * 0.9, dprFloor, base)
      else if (avgFps > 58 && currentDprRef.current < base) next = clamp(currentDprRef.current * 1.05, dprFloor, base)
